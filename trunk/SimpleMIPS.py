...
def divu(inst,states):
	op1 = states.regFile[inst.rs]
	op2 = states.regFile[inst.rt]
	states.lo = Unsign(op1/op2)
	states.hi = Unsign(op1%op2)
...
def srlv(inst,states):
	states.regFile[inst.rd] = states.regFile[inst.rt] >> (states.regFile[inst.rs] & 0x1f)
...
def sllv(inst,states):
	states.regFile[inst.rd] = (states.regFile[inst.rt] << (states.regFile[inst.rs] & 0x1f)) & 0xffffffff
...
def arith(inst,states):
	localmap = {0:sll,6:srlv, 9:jalr,33:addu,35:subu,8:jr,42:slt,24:mult,18:mflo, 43:sltu, 36:opand, 37:opor, 2:srl, 25:multu, 16:mfhi, 39:nor, 3:sra, 4:sllv, 38:xor, 26:div, 27:divu, 7:srav}
...
def lh(inst,states):
	...
	states.regFile[inst.rt] = Sign((states.mem.Read(base,states.pc)&mask)>>(8*offset),16)
